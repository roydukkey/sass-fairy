...
Functions that work on lists are should to accepts parameters that modify list configuration according to the conditions outline below.
...
The `$bracketed` parameter should be accepted when returning a list...
  ...
  1. ...and bracket style cannot be singularly determined from the input.
     ...
     1. [`list.concat()`](/api/list/concat)
     2. [`list.empty()`](/api/list/empty)
     3. etc.

  2. ...that is not a subset of a source list and a potential input value.

     1. [`list.map()`](/api/list/map)
     2. etc.
